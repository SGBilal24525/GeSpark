...
const dataConversionFactors: Record<string, number> = {
  Bit: 1,
  Byte: 8,
  KB: 8 * 1024,
  MB: 8 * 1024 * 1024,
...
const calculateConversion = (value: number, from: string, to: string, toolSlug: string): ConversionResult => {
    if(toolSlug === 'data-converter') {
        const valueInBits = value * dataConversionFactors[from];
        const convertedValue = valueInBits / dataConversionFactors[to];
         return { convertedValue: parseFloat(convertedValue.toPrecision(6)), unit: to };
    }
    return { convertedValue: 0, unit: to };
};
...
export function DataConverter({ tool }: { tool: Tool }) {
                ...
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <Input id="converterValue" type="number" value={converterValue === undefined ? '' : converterValue} onChange={e => setConverterValue(e.target.value === '' ? undefined : parseFloat(e.target.value))} placeholder="e.g., 1024" />
                        </div>
